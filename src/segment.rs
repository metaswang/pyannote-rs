...
pub fn get_segments<P: AsRef<Path>>(
    ...
    Ok(std::iter::from_fn(move || loop {
        ...
        // Normalize window to [-1, 1] before feeding the model
        let mut window_f32 = vec![0.0f32; window.len()];
        knf_rs::convert_integer_to_float_audio(window, &mut window_f32);
        let array = ndarray::Array1::from_vec(window_f32);
